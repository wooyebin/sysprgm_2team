...
void URG_handler(int signo){
    ...
    str_len = recv(clnt_socks[clnt_cnt], (void*)&rinfo, sizeof(rinfo), MSG_OOB);
    ...
    for(i = 0; i<ROOMCOUNT; i++){
        if(strcmp(info[i].roomName,rinfo.roomName) == 0){                
            temp = info[i].cnt - 1;
            if(temp == 0){
                strcpy(info[i].roomName, "\0");
                strcpy(info[i].clnt_names[0] , "\0");
                info[i].clnt_socks[0] = 0;

                temp = info[i].roomnum;
                for(j = temp; j < ROOMCOUNT; j++){
                    if(info[j].roomnum >0){
                        info[j].roomnum--;    
                    }
                }                    
                ...
                strcpy(info[i].clnt_names[temp] , "\0");
                info[i].clnt_socks[temp] = 0;
                info[i].cnt--;
            }
